...
let fs = require('fs');
let InputValidator = require('input-validate');
...
let authenticate = function (username, password, callback) {
    let user = JSON.parse(fs.readFileSync('../myapp/storage/config.json', 'utf8')).user;
...
let updateUser = function (items_to_update) {
    return new Promise(function (resolve, reject) {
        let storage = JSON.parse(fs.readFileSync('../myapp/storage/config.json', 'utf8'));
        //SANITIZE INPUT!

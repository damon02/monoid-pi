            ...
            for (let key in obj) {
                if (obj[key] == "down") {
                    return resolve({
                        success: false,
                        isRunning: false,
                        current_status: "Could not start tap",
                        msg: key + " is down"
                    })
                }
            }

            let startScript = shelljs.exec("sh /home/monoid_dev/raspberry-pi/tap/startScript.sh", {
                async: true
            })
                ...
                isRunning().then(response => {
                    if (response.isRunning) {
                        ...
                        return resolve({
                            success: false,
                            isRunning: false,
                            current_status: "Tap is not running",
                            msg: "Could not start tap!"
                        })
                    }
                })
            })
            ...
            isRunning().then(response => {
                if (response.isRunning) {
                    return resolve({
                        ...
                        isRunning: true,
                        current_status: "Tap is running",
                        msg: "could not stop tap!"
                    })
                } else {
                    return resolve({
                        success: true,
                        isRunning: false,
                        current_status: "Tap not running",
                        msg: ""
                    })
                }
            })
